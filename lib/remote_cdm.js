...
export class GenericRemoteDevice {
    ...
    async generateChallenge(pssh, extra) {
        ...
        if (!selectedRemoteCdm.sg_api_conf) {
            selectedRemoteCdm.sg_api_conf = getDefaultSGConfig(selectedRemoteCdm.type, selectedRemoteCdm.device_name || selectedRemoteCdm.name);
        }
        const remoteCdm = new RemoteCdm(selectedRemoteCdm);

        try {
            const fn = remoteCdm.generateChallenge.bind(remoteCdm, pssh, serverCert);
            const challengeB64 = await (remoteCdm.allowParallel ? fn() : queueCall(fn));
    ...
    async parseLicense(body, extra) {
        ...
        let loadedRequestId = sessionId; // PlayReady
        if (!loadedRequestId) { // Widevine
            ...
            loadedRequestId = uint8ArrayToBase64(licenseObj.id.requestId);
        }
        ...
        try {
            const fn = remoteCdm.parseLicense.bind(remoteCdm, body);
            const keysData = await (remoteCdm.allowParallel ? fn() : queueCall(fn));

            if (!keysData) {
                console.error("[Vineless] No keys received!");
                notifyUser("Remote license parsing failed!", "No keys were received from the remote CDM!");
                return;
            }
...
export class RemoteCdm {
    constructor(dataObj) {
        ...
        this.allowParallel = dataObj.allow_parallel;
...
function getDefaultSGConfig(type, deviceName) {
    if (type === "PLAYREADY") {
        return {
            "headers": {
            ...
            "generateChallenge": [
                ...
                {
                    "method": "POST",
                    "url": `/${deviceName}/get_license_challenge`,
                    "bodyObj": {
                        "privacy_mode": true
                    },
            ...
            "parseLicense": [
                {
                    "method": "POST",
                    "url": `/${deviceName}/parse_license`,
                    "sessionIdKeyName": "session_id",
                    "licenseKeyName": "license_message"
                },
                {
                    "method": "POST",
                    "url": `/${deviceName}/get_keys`,
                    "sessionIdKeyName": "session_id",
                    "contentKeysKeyName": "data.keys"
                },
                {
                    "method": "GET",
                    "url": `/${deviceName}/close/%s`
                }
            ],
            "keyParseRules": {
                "keyKeyName": "key",
        ...
        return {
            ...
            "generateChallenge": [
                ...
                {
                    "method": "POST",
                    "url": `/${deviceName}/get_license_challenge/STREAMING`,
                    "bodyObj": {
                        "privacy_mode": true
                    },
                    "sessionIdKeyName": "session_id",
            ...
            "parseLicense": [
                ...
                {
                    "method": "POST",
                    "url": `/${deviceName}/get_keys/CONTENT`,
                    "sessionIdKeyName": "session_id",
                    "contentKeysKeyName": "data.keys"
                },
                {
...
function queueCall(fn) {
    return new Promise((resolve, reject) => {
        console.log("vlqc", queuedCalls, fn, queuedCalls.length);
        queuedCalls.push({ fn, resolve, reject });
        if (!processingQueue) {
...
async function processQueuedCalls() {
    const { fn, resolve, reject } = queuedCalls.shift();
    try {
        const res = await fn();
        resolve(res);
    } catch (e) {
